  ...
  uint8_t can_id, uint8_t cmd_id, uint16_t option, uint8_t len, uint8_t * data)
{
  ...
  CG_DEBUG_PRINTF("send command [0x%x] \n", id);
  std::this_thread::sleep_for(std::chrono::microseconds(wait_response_time_usec_));
  ++send_count_;
}
...
bool CybergearDriver::receive_motor_data(MotorStatus & mot)
{
  ...
  bool ret = update_motor_status(id, receive_buffer_, len);
  mot = motor_status_;
