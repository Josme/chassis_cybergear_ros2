...
bool CybergearController::send_speed_command(uint8_t id, float speed)
{
  ...
  // drivers_[id].get_vbus();
  return true;
}
float CybergearController::get_vbus(uint8_t id) {
  if (!check_motor_id(id)) return false;
      process_packet();
      drivers_[id].get_vbus();
      auto motor_param =drivers_[id].get_motor_param();
      //printf("motor_param ADDR_VBUS = [%f]\n", motor_param.vbus);
      return motor_param.vbus;
}
